...
import type { MicrosoftCredentialStatus } from "@/lib/services/integrations";

export function MicrosoftCredentialForm({ status }: { status: MicrosoftCredentialStatus }) {
  ...
  async function save(e: React.FormEvent) {
    ...
    const res = await fetch("/api/integrations/microsoft/config", {
      ...
      body: JSON.stringify({ clientId, tenantId, clientSecret: clientSecret || undefined }),
    });
    setBusy(false);
    if (!res.ok) {
    ...
    <form onSubmit={save} className="flex flex-col gap-3">
      {status.source === "env" && (
        <p className="text-xs text-text-faint">
          Currently reading from .env. Saving here will take over from that.
        </p>
      )}
      ...
      <label className="flex flex-col gap-1">
        <span className="font-data text-xs text-text-faint">Tenant ID</span>
        <input
          value={tenantId}
      ...
      <div className="flex items-center gap-1.5 text-xs text-text-faint">
        <ShieldCheck size={13} />
        Encrypted at rest, same as the mailbox connection — never stored in plaintext.
      </div>
      ...
      <button
        type="submit"
        disabled={busy || !clientId.trim() || !tenantId.trim() || (!status.hasSecret && !clientSecret.trim())}
